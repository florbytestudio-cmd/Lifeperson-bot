...
export async function getMonthTransactions(cardId = null) {
  ...
  let q = supabase.from('transactions').select('*, cards(bank,label)').gte('created_at', start.toISOString()).order('created_at', { ascending: false })
...
export async function getUpcomingMemberships(daysAhead = 3) {
  ...
  const { data } = await supabase.from('memberships').select('*').eq('active', true).eq('billing_day', targetDay.getDate())
  return data || []
}
export async function addTask(title, priority = 'media', dueDate = null, notes = null) {
  const { data, error } = await supabase.from('tasks').insert([{ title, priority, due_date: dueDate, notes }]).select().single()
  if (error) throw new Error(error.message)
  return data
}
export async function getTasks(done = false) {
  const { data } = await supabase.from('tasks').select('*').eq('done', done).order('priority').order('created_at', { ascending: false })
  return data || []
}
export async function completeTask(id) {
  const { data, error } = await supabase.from('tasks').update({ done: true, completed_at: new Date().toISOString() }).eq('id', id).select().single()
  if (error) throw new Error(error.message)
  return data
}
...
export async function addNote(content, tags = [], audioUrl = null, transcript = null) {
  const { data, error } = await supabase.from('notes').insert([{ content, tags, audio_url: audioUrl, transcript }]).select().single()
  if (error) throw new Error(error.message)
  return data
}
export async function searchNotes(query) {
  const { data } = await supabase.from('notes').select('*').textSearch('content', query, { type: 'websearch', config: 'spanish' }).order('created_at', { ascending: false }).limit(10)
  return data || []
}
